...
- [x] Lineup floors — 1 RB and 1 WR/TE enforced at flex replacement (148 tests)
...
3. **THEIR$ — pending.** It is the vendor's own auction value, rendered blank in the poc
   because the Draft Sharks auction column is the broken one the spec warns about. Jeff
   did not recognise the column, which is itself the answer: recommendation is to drop it
   and give the width to the PAID write-in box, since the fit against 154 real prices is a
   better market signal than a vendor guess. Confirm before building.
